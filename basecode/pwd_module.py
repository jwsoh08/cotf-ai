...
def password_settings(username):
    ...
    with st.form(key="change_password_form"):
        ...
        repeat_new_password = st.text_input(
            "Repeat New Password", type="password", max_chars=16
        )
...
def reset_passwords(df):
    ...
    selected_users = st.multiselect(
        "You may select multiple users - reset password is '{}'".format(
            DEFAULT_PASSWORD
        ),
    ...
    reset_password = st.text_input(
        "Please enter a temp password for user: (The default password is given)",
        DEFAULT_PASSWORD,
    )

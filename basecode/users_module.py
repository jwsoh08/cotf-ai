...
def link_users_to_app_function_ui(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        selected_function_names = st.multiselect(
            "Select App Functions:", list(app_function_choices.keys())
        )

        # Fetch actual names for levels
        cursor.execute(
            "SELECT level_id, level_name FROM Levels WHERE school_id = ?", (school_id,)
        )
        levels = cursor.fetchall()
        level_choices = {level[1]: level[0] for level in levels}
        selected_level_name = st.selectbox(
            "Select Level (Optional, leave blank for all levels):",
        ...
        cursor.execute(
            "SELECT class_id, class_name FROM Classes WHERE school_id = ? AND (level_id = ? OR ? IS NULL)",
            (school_id, selected_level_id, selected_level_id),
        )
        classes = cursor.fetchall()
        class_choices = {cls[1]: cls[0] for cls in classes}
        selected_class_name = st.selectbox(
            "Select Class (Optional, leave blank for all classes):",
            [""] + list(class_choices.keys()),
        )
        ...
        profile_choices = {
            profile[1]: profile[0] for profile in profiles if profile[0] != 1
        }
        selected_profile_name = st.selectbox(
            "Select Profile (Optional, leave blank for all profiles):",
        ...
        if btn_process:
            ...
            conn.commit()
            st.success(
                f"Users matching the filter have been linked to selected app functions successfully!"
            )
...
def create_prompt_template(user_id):
    ...
    st.warning(
        "Changing the prompt templates will affect the results of your chatbot and other generative functions"
    )
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        current_template_description = next(
            (
                template[2]
                for template in templates
                if template[1] == selected_template_name
            ),
            "",
        )

        new_prompt_description = st.text_area(
            "Update Prompt Description", current_template_description, height=500
        )
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if profile_id == SA:
            ...
            selected_school_name = st.selectbox(
                "Select School:", list(school_choices.keys())
            )
            ...
            for profile_name in multiselect_profile_names:
                if profile_name in profile_choices:
                    ...
                    for user_id in user_ids:
                        ...
                        conn.commit()
                    st.success(
                        f"Prompt templates for users matching the filter have been updated successfully!"
                    )
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if available_vectorstores:
            ...
            selected_vs_id = next(
                (
                    vs["vs_id"]
                    for vs in available_vectorstores
                    if vs["vectorstore_name"] == selected_vs_name
                ),
                None,
            )
            ...
            if not current_profiles_names:
                st.write(
                    f"Knowledge Base: **:blue[{selected_vs_name}]** is only associated to its :green[current owner.]"
                )
            else:
                st.write(
                    f"Knowledge Base: **:blue[{selected_vs_name}]** is currently associated with profiles: **:green[{', '.join(current_profiles_names)}]**"
                )
            ...
            with col2:
                ...
                if remove_all:
                    cursor.execute(
                        "DELETE FROM Profile_VectorStores WHERE vs_id = ?",
                        (selected_vs_id,),
                    )
                    conn.commit()
            ...
            selected_profiles = st.multiselect(
                "Select profiles that can access the KB:", list(profile_options.keys())
            )
...
def add_access_to_vectorstore(cursor, profile_id, vs_id):
    cursor.execute(
        "INSERT INTO Profile_VectorStores (profile_id, vs_id) VALUES (?, ?)",
        (profile_id, vs_id),
    )
...
def vectorstore_selection_interface(user_id):
    ...
    st.write(
        f"Current loaded Knowledge Base: **:blue[{st.session_state.current_model}]**"
    )

    if available_vectorstores:
        # Construct selectbox options
        options = ["Unload KB"] + [
            vs["vectorstore_name"] for vs in available_vectorstores
        ]

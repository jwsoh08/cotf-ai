...
def complete_my_lesson():
    ...
    if plan_action == "Preview Responses":
        ...
        base_filepath = os.path.join(
            "chatbot_response", "crp" + st.session_state.user["username"]
        )
...
def insert_into_data_table(
    date, chatbot_ans, user_prompt, tokens, function_name, value=0
):
...
def search_bot():
    ...
    try:
        if prompt := st.chat_input("Enter your search query"):
            ...
            # st.write(num_tokens)
            insert_into_data_table(
                now.strftime("%d/%m/%Y %H:%M:%S"), full_response, prompt, num_tokens
            )
            # if st.session_state.visuals == True:
            # 	metacognitive_prompter(full_response)
        if st.session_state.download_response_flag == True:
...
def memory_summary_component(prompt):  # currently not in use
    if "memory" not in st.session_state:
        llm = ChatOpenAI(
            model_name=st.session_state.openai_model, temperature=st.session_state.temp
        )

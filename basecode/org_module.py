...
def initialise_admin_account():
    ...
    try:
        ...
        for org in EDU_ORGS:
            cursor.execute(
                "INSERT OR IGNORE INTO Organizations (org_name) VALUES (?)", (org,)
            )
            cursor.execute(
                "SELECT org_id FROM Organizations WHERE org_name = ?", (org,)
            )
        ...
        for profile in SCH_PROFILES:
            cursor.execute(
                "INSERT OR IGNORE INTO Profile (profile_name) VALUES (?)", (profile,)
            )
            cursor.execute(
                "SELECT profile_id FROM Profile WHERE profile_name = ?", (profile,)
            )
            ...
            for org in EDU_ORGS:
                cursor.execute(
                    "INSERT OR IGNORE INTO Profile_Link (profile_id, org_id) VALUES (?, ?)",
                    (profile_id, org_ids[org]),
                )
...
def create_org_structure():
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        num_levels = st.number_input(
            "Enter number of levels in the school:", min_value=1, value=1
        )

        level_names = [
            st.text_input(f"Enter name for Level {i + 1}:") for i in range(num_levels)
        ]
        num_classes_per_level = [
            st.number_input(
                f"Enter number of classes for Level {i + 1}:", min_value=1, value=1
            )
            for i in range(num_levels)
        ]
        class_names = [
            [
                st.text_input(f"Enter name for Class {j + 1} in Level {i + 1}:")
                for j in range(num_classes_per_level[i])
            ]
            for i in range(num_levels)
        ]
        ...
        num_teachers = st.number_input(
            "Enter number of teachers for the school:", min_value=1, value=1
        )
        ...
        if st.button("Submit"):
            ...
            cursor.execute(
                "SELECT school_name FROM Schools WHERE LOWER(school_name) = ?",
                (school_name.lower(),),
            )
            ...
            if existing_school:
                st.warning(
                    f"School {school_name} already exists. Please choose a different name"
                )

            else:
                cursor.execute(
                    "INSERT INTO Schools (org_id, school_name) VALUES ((SELECT org_id FROM Organizations WHERE org_name = ?), ?)",
                    (org_name, school_name),
                )
                student_counter = 1
                for i, level_name in enumerate(level_names):
                    cursor.execute(
                        "INSERT INTO Levels (org_id, school_id, level_name) VALUES ((SELECT org_id FROM Organizations where org_name = ?),(SELECT school_id FROM Schools WHERE school_name = ?), ?)",
                        (org_name, school_name, level_name),
                    )
                    for j, class_name in enumerate(class_names[i]):
                        cursor.execute(
                            "INSERT INTO Classes (org_id, school_id, level_id, class_name) VALUES ((SELECT org_id FROM Organizations where org_name = ?),(SELECT school_id FROM Schools WHERE school_name = ?),(SELECT level_id FROM Levels WHERE level_name = ?), ?)",
                            (org_name, school_name, level_name, class_name),
                        )
                        school_id_query = (
                        ...
                        for k in range(num_students_per_class[i][j]):
                            ...
                            cursor.execute(
                                "INSERT INTO Users (username, password, profile_id, school_id, class_id, org_id, level_id) VALUES (?, ?, ?, ?, (SELECT class_id FROM Classes WHERE class_name = ?), (SELECT org_id FROM Organizations WHERE org_name = ?), (SELECT level_id FROM Levels WHERE level_name = ?))",
                                (
                                    student_username,
                                    hash_password(STU_PASS),
                                    STU,
                                    school_id,
                                    class_name,
                                    org_name,
                ...
                for t in range(num_teachers):
                    ...
                    cursor.execute(
                        "INSERT INTO Users (username, password, profile_id, school_id, org_id) VALUES (?, ?, ?, ?, (SELECT org_id FROM Organizations WHERE org_name = ?))",
                        (
                            teacher_username,
                            hash_password(TCH_PASS),
                            TCH,
                            school_id,
                            org_name,
                        ),
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if show_unassigned:
            ...
            selected_teacher = st.selectbox(
                "Select Unassigned Teacher:", unassigned_teachers
            )
        else:
            ...
            selected_teacher = st.selectbox(
                "Select Teacher to Remove or Reassign:", assigned_teachers
            )
            ...
            if selected_teacher:
                ...
                assigned_classes_to_teacher = [
                    class_[0] for class_ in cursor.fetchall()
                ]
                st.write(
                    f"Classes currently assigned to {selected_teacher}: {', '.join(assigned_classes_to_teacher)}"
                )
        ...
        if not remove_assignment:
            ...
            cursor.execute(
                "SELECT class_name FROM Classes WHERE school_id = ?", (school_id,)
            )
            all_classes = [class_[0] for class_ in cursor.fetchall()]
            selected_classes = st.multiselect(
                "Select Classes (For Reassignment or Assignment):",
                all_classes,
                default=all_classes[0],
            )
        ...
        if btn_process:
            cursor.execute(
                "SELECT user_id FROM Users WHERE username = ?", (selected_teacher,)
            )
            ...
            if remove_assignment:
                ...
                cursor.execute(
                    "UPDATE Users SET class_id = NULL WHERE user_id = ?", (teacher_id,)
                )
            else:
                ...
                for index, new_class in enumerate(selected_classes):
                    cursor.execute(
                        "SELECT class_id, level_id FROM Classes WHERE class_name = ?",
                        (new_class,),
                    )
                    ...
                    if not already_assigned:
                        cursor.execute(
                            ...
                            (teacher_id, school_id, level_id, class_id),
                        )
...
def change_teacher_profile_ui(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        cursor.execute(
            "SELECT profile_id, profile_name FROM Profile WHERE profile_id != ? AND profile_id != ?",
            (STU, SA),
        )
        ...
        sch_profile_ids = [
            profile_name_to_id[profile_name]
            for profile_name in SCH_PROFILES
            if profile_name in profile_name_to_id
        ]
		...
		""".format(
                ",".join("?" * len(sch_profile_ids))
            ),
            [school_id] + sch_profile_ids,
        )
        ...
        for teacher in teachers:
            ...
            teacher_profile_selections[user_id] = profile_name_to_id[
...
def reassign_student_ui(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        cursor.execute(
            "SELECT DISTINCT level_id FROM Classes WHERE school_id = ?", (school_id,)
        )
        ...
        cursor.execute(
            "SELECT class_name FROM Classes WHERE school_id = ? AND level_id = ?",
            (school_id, selected_level),
        )
        ...
        cursor.execute(
            "SELECT username FROM Users WHERE profile_id = ? AND school_id = ? AND class_id IN (SELECT class_id FROM Classes WHERE class_name = ?)",
            (STU, school_id, selected_class),
        )
        students_in_class = [student[0] for student in cursor.fetchall()]
        selected_students = st.multiselect(
            "Select Students to Reassign:", students_in_class
        )
        ...
        cursor.execute(
            "SELECT class_name FROM Classes WHERE school_id = ? AND class_name != ?",
            (school_id, selected_class),
        )
        all_classes_for_reassignment = [class_[0] for class_ in cursor.fetchall()]
        new_class = st.selectbox(
            "Select New Class for Reassignment:", all_classes_for_reassignment
        )
        ...
        if btn_reassign:
            ...
            cursor.execute(
                "SELECT class_id FROM Classes WHERE class_name = ?", (new_class,)
            )
            ...
            conn.commit()
            st.success(
                f"Selected students have been reassigned to {new_class} successfully!"
            )
...
def process_user_profile(profile_name):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if profile_name == SA:  # Using the global SA constant
            ...
            org_name_selected = st.selectbox(
                "Choose an Organization:", [org[0] for org in all_organization_names]
            )
            ...
            cursor.execute(
                "SELECT school_name FROM Schools WHERE org_id = ?", (org_id,)
            )
            ...
            if not schools_in_org:
                return (
                    None,
                    "No schools found for the selected organization. Please create a school first.",
                )

            school_selected = st.selectbox(
                "Choose a School within the Organization:",
                [school[0] for school in schools_in_org],
            )
            ...
            return (
                school_id,
                f"Selected school ID: {school_id} within Organization: {org_name_selected}",
            )
        else:  # Using the global AD constant
            return (
                st.session_state.user["school_id"],
                f"User's school ID: {st.session_state.user['school_id']}",
            )
...
def add_level(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if st.button("Add Level"):
            ...
            cursor.execute(
                "SELECT level_name FROM Levels WHERE school_id = ? AND level_name = ?",
            ...
            if existing_level:
                st.warning(
                    f"Level {level_name} already exists for this school. Please choose a different name."
                )
            else:
                cursor.execute(
                    "INSERT INTO Levels (org_id, school_id, level_name) VALUES (?, ?, ?)",
                    (org_id, school_id, level_name),
                )
                conn.commit()
...
def add_class(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        cursor.execute(
            "SELECT DISTINCT level_name FROM Levels WHERE school_id = ?", (school_id,)
        )
        ...
        if st.button("Add Class"):
            ...
            cursor.execute(
                "SELECT class_name FROM Classes WHERE school_id = ? AND level_id = (SELECT level_id FROM Levels WHERE level_name = ? AND school_id = ?) AND class_name = ?",
                (school_id, level_name, school_id, class_name),
            )
            existing_class = cursor.fetchone()

            if existing_class:
                st.warning(
                    f"Class {class_name} already exists for this level and school. Please choose a different name."
                )
            else:
                cursor.execute(
                    "INSERT INTO Classes (org_id, school_id, level_id, class_name) VALUES (?, ?, (SELECT level_id FROM Levels WHERE level_name = ? AND school_id = ?), ?)",
                    (org_id, school_id, level_name, school_id, class_name),
                )
                conn.commit()
...
def add_user(school_id):
    try:
        ...
        with sqlite3.connect(WORKING_DATABASE) as conn:
            ...
            # Retrieve org_id based on school_id
            cursor.execute(
                "SELECT org_id FROM Schools WHERE school_id = ?", (school_id,)
            )
            org_id = cursor.fetchone()[0]
            ...
            username = st.text_input(
                f"Enter {user_type.lower()} username (Do not put tch1/stu1 at the start as it will be appended automatically):"
            )
            ...
            if user_type == "Teacher":
                ...
                if st.button(f"Add {user_type}"):
                    ...
                    cursor.execute(
                        "INSERT INTO Users (username, password, profile_id, school_id, class_id, org_id, level_id) VALUES (?, ?, ?, ?, (SELECT class_id FROM Classes WHERE class_name = ? AND school_id = ?), ?, (SELECT level_id FROM Levels WHERE level_name = (SELECT level_name FROM Classes WHERE class_name = ? AND school_id = ?) AND school_id = ?))",
                        (
                            student_username,
                            hash_password(STU_PASS),
                            STU,
...
def remove_user(school_id):
    ...
    cursor.execute(
        "SELECT user_id, username FROM Users WHERE school_id=? AND user_id NOT IN (?)",
        (school_id, 1),
    )
    users = cursor.fetchall()
    ...
    if st.button(f"Delete User with ID {user_id}"):
        ...
        if confirmation:
            try:
                ...
                cursor.execute(
                    "DELETE FROM Prompt_Templates WHERE user_id=?", (user_id,)
                )
                cursor.execute(
                    "DELETE FROM App_Functions_Link WHERE user_id=?", (user_id,)
                )
                cursor.execute("DELETE FROM BotSettings WHERE user_id=?", (user_id,))
                cursor.execute(
                    "DELETE FROM User_VectorStores WHERE user_id=?", (user_id,)
                )
...
def delete_if_no_association(cursor, table_name, column_name, value):
    ...
    for associated_table in associated_tables.get(table_name, []):
        cursor.execute(
            f"SELECT COUNT(*) FROM {associated_table} WHERE {column_name}=?", (value,)
        )
        count = cursor.fetchone()[0]
        if count > 0:
            return False
...
def delete_class(class_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        return (
            ...
            if success
            else "Cannot delete: Class has associated records."
        )
...
def delete_level(level_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        return (
            ...
            if success
            else "Cannot delete: Level has associated records."
        )
...
def delete_school(school_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        return (
            ...
            if success
            else "Cannot delete: School has associated records."
        )
...
def streamlit_delete_interface():
    ...
    choice = st.selectbox(
        "Which entity do you want to delete?", ["Class", "Level", "School"]
    )

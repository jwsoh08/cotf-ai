...
def bot_settings():
    with st.form(key="sliders_form"):
        ...
        temp = st.slider(
            "Temp", min_value=0.0, max_value=1.0, value=st.session_state.temp, step=0.01
        )
        ...
        chat_memory = st.slider(
            ...
            min_value=0,
            max_value=10,
            value=st.session_state.k_memory,
            step=1,
        )
...
def propagate_bot_settings(
    profile_id, temp, presence_penalty, frequency_penalty, selected_school_id
):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        cursor = conn.cursor()
        ...
        if profile_id == SA:
            ...
            cursor.execute(
                "SELECT user_id FROM Users WHERE school_id = ?", (selected_school_id,)
            )
...
def bot_settings_interface(profile_id, school_id=None):
    with st.form(key="sliders_form"):
        ...
        presence_penalty = st.slider(
            ...
            min_value=-2.0,
            max_value=2.0,
            value=float(st.session_state.presence_penalty),
            step=0.01,
        )
        frequency_penalty = st.slider(
            "Frequency Penalty",
            min_value=-2.0,
            max_value=2.0,
            value=float(st.session_state.frequency_penalty),
            step=0.01,
        )
        ...
        if submit_button:
            ...
            st.session_state.frequency_penalty = frequency_penalty
            store_bot_settings(
                st.session_state.user["id"], temp, presence_penalty, frequency_penalty
            )
            st.success("Parameters saved!")
            if should_propagate:
                with sqlite3.connect(WORKING_DATABASE) as conn:
                    ...
                    if profile_id == SA:
                        ...
                        selected_school_name = st.selectbox(
                            "Select School for Propagation:",
                            list(school_choices.keys()),
                        )
                        ...
                        st.write(
                            f"You're set to propagate settings to school with ID: {school_id}"
                        )
                        selected_school_id = school_choices[selected_school_name]
                        propagate_bot_settings(
                            profile_id,

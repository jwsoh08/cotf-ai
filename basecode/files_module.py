...
def save_file_to_db(
    org_id, user_id, file_name, file_content, metadata, subject, topic, sharing_enabled
):
    conn = sqlite3.connect(WORKING_DATABASE)
    cursor = conn.cursor()
    ...
    if not topic_id_result:
        cursor.execute(
            "INSERT INTO Topic (org_id, topic_name) VALUES (?, ?)", (org_id, topic)
        )
...
def docs_uploader():
    ...
    meta = st.text_input(
        "Please enter your document source (Default is MOE):", max_chars=20
    )
    ...
    if profile in [SA, AD]:
        ...
        for file_name in file_names:
            ...
            if cursor.rowcount == 0:
                st.error(
                    f"Unable to delete file '{file_name}' that is not owned by you."
                )
